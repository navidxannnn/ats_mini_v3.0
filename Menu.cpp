...
//
// Restart confirmation ("No" is always the safe default so an
// accidental click on the menu item can never restart the device)
//
uint8_t restartIdx = 0;
static const char *restartDesc[] = { "No", "Yes" };
...
//
// Move the Yes/No selection for the restart confirmation
//
static void doRestart(int16_t enc)
{
  restartIdx = wrap_range(restartIdx, enc, 0, LAST_ITEM(restartDesc));
}
...
static void clickSettings(int cmd, bool shortPress)
{
  ...
  switch(cmd)
  {
    ...
    case MENU_RESTART:
      restartIdx = 0;           // Always default to "No"
...
bool doSideBar(uint16_t cmd, int16_t enc, int16_t enca)
{
  ...
  switch(cmd)
  {
    ...
    case CMD_RESTART:    doRestart(scrollDirection * enc);break;
...
//
// Handle click on the restart confirmation screen.
// Only restarts if "Yes" was selected; "No" (the default) just
// backs out to the Settings menu, same as any other cancelled item.
//
static void clickRestart(bool shortPress)
{
  if(restartIdx==1)
  {
    ESP.restart();
...
//
// Handle click while the Waterfall view is open: any click exits it
// and restores the original tuned frequency / mute state.
//
...
bool clickHandler(uint16_t cmd, bool shortPress)
{
  switch(cmd)
  {
    ...
    case CMD_RESTART:   clickRestart(shortPress);break;
...
static void drawRestartMenu(int x, int y, int sx)
{
  drawCommon("Restart Device?", x, y, sx, true);

  int count = ITEM_COUNT(restartDesc);
  for(int i=-2 ; i<3 ; i++)
  {
    if(i==0) {
      drawZoomedMenu(restartDesc[abs((restartIdx+count+i)%count)]);
      spr.setTextColor(TH.menu_hl_text, TH.menu_hl_bg);
    } else {
      spr.setTextColor(TH.menu_item);
    }

    // Prevent repeats for a 2-item list
    if((restartIdx+i) < 0 || (restartIdx+i) >= count) continue;

    spr.setTextDatum(MC_DATUM);
    spr.drawString(restartDesc[abs((restartIdx+count+i)%count)], 40+x+(sx/2), 64+y+(i*16), 2);
  }
}
...
void drawSideBar(uint16_t cmd, int x, int y, int sx)
{
  ...
  switch(cmd)
  {
    ...
    case CMD_RESTART:    drawRestartMenu(x, y, sx);break;
