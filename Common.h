...
#define RECEIVER_NAME  "ATS-Mini"                  // Technical device ID: BLE name + Wi-Fi AP SSID (kept unchanged)
#define FIRMWARE_EDITION "New V3.0"                // Marketing/edition name shown on Splash, About and Settings screens
...
#define VER_APP        300  // Firmware version (New V3.0)
...
// Scan.c
#define SCAN_POINTS 200 // Number of frequencies covered by one sweep (shared with Waterfall.c)
...
bool scanContinuousTick();  // Call once per loop(); returns true when a new sweep row is ready
void scanContinuousStop();
uint8_t scanGetPointRSSI(uint16_t index); // Raw RSSI (SI4735 units) of the last completed sweep
