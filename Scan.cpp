...
//
// Non-blocking continuous sweep, driven one small step per call.
// Used by the Waterfall view: unlike scanRun() (which blocks until an
// entire sweep finishes), this lets the main loop() keep servicing the
// encoder/button/UI between steps. Every time one sweep completes, it
// is immediately restarted, so calling this once per loop() iteration
// produces a continuous stream of real, measured sweeps.
//
static bool continuousActive = false;
static uint16_t continuousCenterFreq;
static uint16_t continuousStep;

void scanContinuousStart(uint16_t centerFreq, uint16_t step)
{
  rx.setMaxDelaySetFrequency(currentMode == FM ? TUNE_DELAY_FM : TUNE_DELAY_AM_SSB);
  muteOn(MUTE_TEMP, true);
  seekStop = false;

  continuousCenterFreq = centerFreq;
  continuousStep       = step;
  continuousActive     = true;
  scanInit(centerFreq, step);
}

bool scanContinuousTick()
{
  if(!continuousActive) return(false);

  if(scanTickTime()) return(false); // Sweep still running, nothing new yet

  // This sweep just completed - a new row of data is ready to read.
  // Immediately kick off the next sweep around the same center frequency.
  scanInit(continuousCenterFreq, continuousStep);
  return(true);
}

void scanContinuousStop()
{
  ...
  // Caller is responsible for restoring the frequency that was tuned
  // before the continuous sweep started (see waterfallExit()).
}

uint8_t scanGetPointRSSI(uint16_t index)
{
  return(index<SCAN_POINTS ? scanData[index].rssi : 0);
}

//
// Run entire scan once (blocking)
//
