...
// Minimum time the splash screen stays visible (ms). Kept short: the
// real init steps below (SI4732 detect, filesystem, preferences) already
// take a comparable amount of time, so this rarely adds any extra wait.
#define SPLASH_MIN_TIME  800
...
void setup()
{
  ...
  // Show the New V3.0 splash logo as soon as the display is ready.
  // Skipped if the encoder button is held (preferences-reset path below
  // needs the screen for its own text immediately).
  uint32_t splashStart = millis();
  bool     splashShown = false;
  if(digitalRead(ENCODER_PUSH_BUTTON) != LOW)
  {
    tft.setSwapBytes(true);
    tft.pushImage(0, 0, SPLASH_WIDTH, SPLASH_HEIGHT, splashImage);
    ledcWrite(PIN_LCD_BL, currentBrt);   // Backlight on so the logo is actually visible
    splashShown = true;
  }
  ...
  // Keep the splash logo up for at least SPLASH_MIN_TIME total. In
  // practice the init steps above already take close to this long, so
  // this rarely waits at all; it only tops up the remainder, if any.
  if(splashShown)
  {
    uint32_t elapsed = millis() - splashStart;
    if(elapsed < SPLASH_MIN_TIME) delay(SPLASH_MIN_TIME - elapsed);
  }
...
void loop()
{
  ...
  // Tick the continuous Waterfall sweep, if that view is open
  if(currentCmd == CMD_WATERFALL) needRedraw |= waterfallTickTime();
